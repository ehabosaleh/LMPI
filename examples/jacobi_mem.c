...
int main(int argc, char *argv[]) {
	int rank, size,len;
   	...
   	double*column_left=LMPI_Register(local_N,MPI_DOUBLE);
   	double*column_right=LMPI_Register(local_N,MPI_DOUBLE);
   	double*column_left_recv=NULL;
   	double*column_right_recv=NULL;

   	double*row_top=LMPI_Register(local_N,MPI_DOUBLE);
   	double*row_bottom=LMPI_Register(local_N,MPI_DOUBLE);
   	double *row_top_recv=NULL;
   	double *row_bottom_recv=NULL;
		...
		init_time=MPI_Wtime();
      		...
      		if (left != MPI_PROC_NULL) {
	    		...
	    		for (int i = 1; i <= local_N; i++)
                	...
                	LMPI_Isend(column_left, local_N, MPI_DOUBLE, left, iter+0, LMPI_COMM_WORLD, &reqs[req_count++]);
            		LMPI_Irecv((void**)&column_right_recv, local_N, MPI_DOUBLE, left, iter+1, LMPI_COMM_WORLD, &reqs[req_count++]);
        	}
      		...
      		if (right != MPI_PROC_NULL) {
            		...
            		LMPI_Isend(column_right, local_N, MPI_DOUBLE, right, iter+1, LMPI_COMM_WORLD, &reqs[req_count++]);
            		LMPI_Irecv((void**)&column_left_recv, local_N, MPI_DOUBLE, right, iter+0, LMPI_COMM_WORLD, &reqs[req_count++]);
            	}

        	if (top != MPI_PROC_NULL) {
			for (int i = 1; i <=local_N; i++)
                     		row_top[i-1]=u[1][i];
            		LMPI_Isend(row_top, local_N, MPI_DOUBLE, top, iter+2, LMPI_COMM_WORLD, &reqs[req_count++]);
            		LMPI_Irecv((void**)&row_bottom_recv, local_N, MPI_DOUBLE, top, iter+3, LMPI_COMM_WORLD, &reqs[req_count++]);
        	}

		if (bottom != MPI_PROC_NULL) {
			for (int i = 1; i <=local_N; i++)
                     		row_bottom[i-1]=u[rows-2][i];

            		LMPI_Isend(row_bottom, local_N, MPI_DOUBLE, bottom, iter+3, LMPI_COMM_WORLD, &reqs[req_count++]);
            		LMPI_Irecv((void**)&row_top_recv, local_N, MPI_DOUBLE, bottom, iter+2, LMPI_COMM_WORLD, &reqs[req_count++]);
        	}
	...
	//printf("host %s rank %d time %f \n",process_name,rank,(MPI_Wtime()-init_time)*1e6);
		...
		if(left!=MPI_PROC_NULL)
                        for (int i = 1; i <= local_N; i++)
                                u[i][0]=column_right[i-1];
         	if(right!=MPI_PROC_NULL)
                        for (int i = 1; i <=local_N; i++)
                                u[i][cols-1]=column_left[i-1];
	 	if(top!=MPI_PROC_NULL)
                        for (int i = 1; i <= local_N; i++)
                                u[1][i]=row_top[i-1];
         	if(bottom!=MPI_PROC_NULL)
                        for (int i = 1; i <=local_N; i++)
                                u[rows-2][i]=row_bottom[i-1];
	...
	double solve_time=MPI_Wtime();
   	...
   	for (iter = 0; iter < MAX_ITER; iter++) {
		...
		u = u_new;
        	...
        	MPI_Allreduce(&max_diff, &global_max_diff, 1, MPI_DOUBLE, MPI_MAX, LMPI_COMM_WORLD);
	...
	MPI_Reduce(&solve_time,&solve_time_avr,1,MPI_DOUBLE,MPI_SUM,0,LMPI_COMM_WORLD);
