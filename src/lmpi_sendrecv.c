...
int LMPI_Irecv(void **data, int count, MPI_Datatype datatype, int source, int tag, MPI_Comm comm, LMPI_Request *request){
	...
	request->request_status=LMPI_STATUS_PENDING;
        request->count=count;
        request->mem_offset= LMPI_Alloc(count, datatype);;
        request->local_rank=local_rank;
        *data = (char*)shm_buf_recv[local_rank]+request->mem_offset;
...
int LMPI_Isend(void*data, int count, MPI_Datatype datatype, int dst, int tag,  MPI_Comm comm, LMPI_Request *request){
	int sizeofdata=0;
	int rank=0;
        MPI_Comm_rank(comm,&rank);
	...
	request->world_src=world_src;
        request->mem_offset=(MPI_Aint)((char*)data - (char*)shm_buf_send[local_rank]);
	/* Used only with one-sided progress thread
	MPI_Win_attach(win, data, count*sizeofdata);
        MPI_Get_address(data, &remote_addr);
        request->remote_win_addr=remote_addr;
        //debug_log("Sender request Remote address %lld",remote_addr);
        */
	request->send_buffer=data;
        request->mem_offset=shm_offset_send;
        request->index=enqueue_request(*request);
